...
import { LoginModalService } from './../../../compontents/widget/login-modal/login-modal';
...
export class UfastCodeInterceptor implements HttpInterceptor {
    ...
    intercept(req: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
        ...
        const newReq = req.clone({
            headers: req.headers.append('x-site-code', environment.otherData.sysSite)
                .append('Cache-Control', 'no-cache,no-store')
                .append('Pragma', 'no-cache')
                .append('x-channel-code', environment.otherData.defaultPath)
        });
